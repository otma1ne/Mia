...
import type { TrainerSessionEvent } from '@/app/actions/trainer-dashboard'
import { getTrainerSessions } from '@/app/actions/trainer-dashboard'
import { Button } from '@/components/ui/button'
import { Card } from '@/components/ui/card'
import { Badge } from '@/components/ui/badge'
import {
  ChevronLeft, ChevronRight, CalendarDays, List, Clock, MapPin,
} from 'lucide-react'
...
interface TrainerScheduleClientProps {
  initialSessions: TrainerSessionEvent[]
  refDateStr: string
  view: 'week' | 'list'
}

export default function TrainerScheduleClient({
  initialSessions,
  refDateStr,
  view: initialView,
}: TrainerScheduleClientProps) {
  const router   = useRouter()
  const pathname = usePathname()
  const params   = useSearchParams()

  const [sessions, setSessions]   = useState<TrainerSessionEvent[]>(initialSessions)
  ...
  function navigate(newRef: Date) {
    ...
    startTransition(async () => {
      const newWeekStart = startOfWeek(newRef, { weekStartsOn: 1 })
      const newWeekEnd   = endOfWeek(newRef, { weekStartsOn: 1 })
      const fresh = await getTrainerSessions({ from: newWeekStart, to: newWeekEnd })
      setSessions(fresh)
    })
  }
          ...
          <div className="grid border-b" style={{ gridTemplateColumns: '3rem 1fr 1fr 1fr 1fr 1fr 1fr 1fr' }}>
            ...
            {days.map(day => (
              ...
              >
                ...
                <p className={cn('text-sm font-semibold mt-0.5', isToday(day) && 'text-primary')}>
                  {format(day, 'd')}
                </p>
              </div>
            ))}
                      ...
                      {daySessions.map(session => {
                        ...
                        return (
                          <div
                            key={session.id}
                            className={cn(
                              'absolute rounded border p-1.5 overflow-hidden text-xs leading-snug pointer-events-auto',
                              colour.chip
                            )}
      ...
      {view === 'list' && (
        <div className="flex flex-col gap-3">
          {sessionsThisWeek.length === 0 ? (
            <div className="flex items-center justify-center rounded-xl ring-1 ring-foreground/10 bg-card py-12">
                    ...
                    {daySessions.map(session => {
                      ...
                      return (
                        <div key={session.id} className={cn(
                          ...
                          <span className={cn('rounded-full px-2 py-0.5 text-xs font-medium border shrink-0', colour.chip)}>
