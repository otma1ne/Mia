...
import type { StudentSessionEvent } from '@/app/actions/student-dashboard'
...
interface StudentScheduleClientProps {
  initialSessions: StudentSessionEvent[]
  refDateStr: string
  view: 'week' | 'list'
}
...
export default function StudentScheduleClient({
  ...
  const [sessions, setSessions] = useState<StudentSessionEvent[]>(initialSessions)
  ...
  function navigate(newRef: Date) {
    ...
    startTransition(async () => {
      ...
      const fresh = await getStudentSessions({ from: newWeekStart, to: newWeekEnd })
      setSessions(fresh)
    })
  }
